...
  // 🔥 NEW: Direct Browser-to-Drive Upload (No Size Limit)
...
      // 1. Backend से Upload Link मांगें
...
      if (!authResponse.ok) throw new Error('Failed to get upload link');
      const { uploadUrl } = await authResponse.json();

      // 2. अब सीधे Google Drive पर फाइल भेजें (Direct PUT Request)
      setTransferSpeed('Starting High-Speed Upload... 🚀');
      
      const xhr = new XMLHttpRequest();
      xhr.open('PUT', uploadUrl, true);
      // Google को content-type बताना ज़रूरी है
...
          setTransferProgress(percent);
          setTransferSpeed(`Uploading directly to Drive... ☁️ ${percent}%`);
        }
      };
...
          try {
            // अपलोड खत्म होने पर Google फाइल का डेटा देता है
            const result = JSON.parse(xhr.responseText);
            setCloudLink(result.webViewLink); // User को लिंक दिखाएं
            setTransferSpeed('Upload Complete! 🎉 (Saved to 2TB Drive)');
            setTransferProgress(100);
          } catch (e) {
             // कभी-कभी Google पूरा response नहीं देता, लेकिन अपलोड हो जाता है
             setTransferSpeed('Upload Done! Check your Drive.');
             setTransferProgress(100);
          }
        } else {
